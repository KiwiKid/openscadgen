...
screw_head_hole_radius_size = 5.2; //max(screw_hole_radius_size*1.5, 5);
...
module label_text(txt) {
    if (!is_undef(txt)) {
        ...
        text_size = 2;  // Size of the text
        text_height = 1; // Height of the extruded text
        ...
        translate([-total_width / 2, 4, -button_width-0.1])
...
difference(){
    ...
    translate([0, 0, -button_radius+button_height])
