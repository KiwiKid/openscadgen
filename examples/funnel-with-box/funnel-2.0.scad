...
holderHeight = 10;
holderOffset = larger_OD/1.5 -13;
cylinderRotationX = 7;
cylinderRotationY = -7;
//attachment
translate([0,holderOffset,10])
rotate([cylinderRotationX,cylinderRotationY,0])
difference(){
    cuboid([holderHeight,30,10], rounding=2);
    
    
    translate([0,0,-7])
    union(){
        cylinder(h=holderHeight, r=2);
        cylinder(h=holderHeight+10, r=1);
    }
 }
 holderCylinderHeight = 15;
 holderCylinderRadius = 10;
 holderCylinderLoc = [0,holderOffset-24,0];
 
 translate(holderCylinderLoc)
 difference(){
         cylinder(h=holderCylinderHeight, r=holderCylinderRadius);
         translate([0,0,-1])
        cylinder(h=holderCylinderHeight+2, r=holderCylinderRadius-1);
